...
function drawScene() {
	...
	if (p) {
		if (p.inMoveX == 1 || p.inMoveX == -1) {
			p.inMoveX = 0;
		}
		if (p.inMoveY == 1 || p.inMoveY == -1) {
			p.inMoveY = 0;
		}
...
function drawHUDEnd() {
	...
	t = "1024 MOVES";
	drawText((window.innerWidth / 2) - (12 * 15 / 2) * (t.length / 2), 100, t, 15);
...
function drawHUDHome() {
	...
	drawText((window.innerWidth / 2) - 42 * (t.length / 2), 500 + Math.sin(frame / 64) * 10, t, 7);
...
var tuto = {
	1: [
		[{
			...
			y: 2,
			z: -8
		}, "this is you. Move with arrow keys", 4000],
		[{
			x: 14,
			y: 2,
			z: -8
		}, "Watch out for holes", 3000],
		[{
			x: 22,
			y: 2,
			z: -8
		}, "this is your goal, get the orb", 4000]
	],
	2: [
		[{
			x: 12,
	...
	5: [
		[{
			...
			y: -2,
			z: -6
		}, "this is a switch, which modifies something in the stage", 3000]
	],
	6: [
		[{
			x: 2,
			y: -2,
			z: -6
		}, "this is another sort of trigger, have to keep it pressed", 3000]
	],
	7: [
		[{
			x: 8,
			y: 6,
			z: 0
		}, "this is another player. Switch between players with space key", 4000]
	],
	12: [
		[{
			x: 8,
			y: -2,
			z: -8
		}, "this block cannot be moved, but it is climbable", 4000]
	]
};
